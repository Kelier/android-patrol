...
public class UserEditText extends EditText {
    private final String TAG = "UserEditText";
    private Drawable dRight;
    private Drawable dLeft;
    private Rect rBounds;
    private Drawable mIconPerson;
    public UserEditText(Context paramContext) {
        super(paramContext);

        initEditText();
    }

    public UserEditText(Context paramContext, AttributeSet paramAttributeSet) {
        super(paramContext, paramAttributeSet);
        initEditText();
    }

    public UserEditText(Context paramContext, AttributeSet paramAttributeSet, int paramInt) {
    ...
    private void initEditText() {
        ...
        addTextChangedListener(new TextWatcher() { // 对文本内容改变进行监听
            ...
            public void onTextChanged(CharSequence paramCharSequence, int paramInt1, int paramInt2, int paramInt3) {
                UserEditText.this.setEditTextDrawable();
            }
        });
    ...
    public void setEditTextDrawable() {
        if (getText().toString().length() == 0) {
            ...
            dRight=getResources().getDrawable(R.drawable.icon_clear);
            dRight.setBounds(-20, 0, 35, 55);
